...
from .query_processor import preprocess_query, classify_query_type
...
class GraphRAGQueryEngine:
    ...
    def query(self, user_query: str, query_type: str = "auto", max_results: int = 10) -> Dict[str, Any]:
        ...
        # 1. Preprocess query using the new query_processor module
        preprocessed = preprocess_query(user_query, max_results)
        if query_type == "auto":
        ...
        # 4. Aggregate results using the new result_aggregator module
        aggregated_results = aggregate_search_results(entities, {"subgraph": []}, preprocessed)
        formatted_context = format_results_for_llm(aggregated_results)
        
        # 5. Route to appropriate handler with reasoning
        if query_type == "drug_repurposing":
            return self._handle_drug_repurposing_with_reasoning(user_query, entities, formatted_context, aggregated_results)
        elif query_type == "mechanism_explanation":
            return self._handle_mechanism_with_reasoning(user_query, entities, formatted_context, aggregated_results)
        elif query_type == "hypothesis_testing":
            return self._handle_hypothesis_with_reasoning(user_query, entities, formatted_context, aggregated_results)
        else:
            return self._handle_general_with_reasoning(user_query, entities, formatted_context, aggregated_results)
